...
+ (NSString *)uid {
    return [[[APDeviceUID alloc] initWithKey:@"deviceUID"] uid];
}
 ...
 */
- (void)save {
  if (![APDeviceUID valueForUserDefaultsKey:_uidKey]) {
    [APDeviceUID setValue:_uid forUserDefaultsKey:_uidKey];
  }
  if (![APDeviceUID valueForKeychainKey:_uidKey service:_uidKey]) {
    [APDeviceUID setValue:_uid forKeychainKey:_uidKey inService:_uidKey];
  }
}
